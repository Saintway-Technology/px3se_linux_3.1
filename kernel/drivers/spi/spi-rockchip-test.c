...
/* how to test spi
* echo write 2 10 255 > /dev/spi_misc_test
* echo write 2 10 255 init.rc > /dev/spi_misc_test
* echo read 2 10 255 > /dev/spi_misc_test
* echo setspeed 2 1000000 > /dev/spi_misc_test
*/
...
static ssize_t spi_test_read(struct file *file, char __user *userbuf, size_t bytes, loff_t *off)
{
        int ret;
        int i=0;
        char *txbuf = NULL, *rxbuf = NULL;
        printk("bytes = %zu",bytes);
        txbuf = kzalloc(1, GFP_KERNEL);
        rxbuf = kzalloc(bytes, GFP_KERNEL);
	ret = copy_from_user(txbuf,userbuf,1);
        if (!rxbuf) {
                printk("spi read alloc buf size  fail\n");
                return -1;
        }
	printk("hans : %x\n",txbuf[0]);
        spi_write_then_read_slt(0, txbuf, 1, rxbuf, bytes);
        for (i = 0; i < bytes; i++){
                printk("lwh: read rxbuf[%d] =0x%x\n", i, rxbuf[i]);
                mdelay(1);
        }
        ret = copy_to_user(userbuf, rxbuf, bytes);
        kfree(txbuf);
        kfree(rxbuf);
        return ret;
}

static ssize_t spi_data_write(struct file *file,const char __user *userbuf, size_t bytes, loff_t *off)
{
	char *txbuf = NULL;
	int ret;
	int i = 0;
	txbuf = kzalloc(bytes, GFP_KERNEL);
	ret = copy_from_user(txbuf,userbuf,bytes);
	printk("lwh: userbuffer = %s ,txbuf = %s ,bytes = %zu\r\n",userbuf,txbuf,bytes);
	for (i = 0; i < bytes; i++){
                printk("lwh: write txbuf[%d] =0x%x\n", i, txbuf[i]);
                mdelay(1);
        }
	spi_write_slt(0, txbuf, bytes);
	kfree(txbuf);
	return ret;
}
			...
			const char __user *buf, size_t n, loff_t *offset)
{
	...
	if (!strcmp(cmd, "setspeed")) {
		...
		rxbuf = kzalloc(size+2, GFP_KERNEL);
		if (!rxbuf) {
		...
		txbuf[0] = 0x80 | times;
		printk("Sam: read_chl = 0x%x\n!",txbuf[0]);
		start_time = ktime_get();
	
		spi_write_then_read_slt(id, txbuf, 1, rxbuf, size+2);
	
		end_time = ktime_get();
		for (i = 0; i < size; i++){
			printk("Sam: read rxbuf[%d] =0x%x\n", i, rxbuf[i+2]);
			mdelay(1);
		}
		cost_time = ktime_sub(end_time, start_time);
		us = ktime_to_us(cost_time);
