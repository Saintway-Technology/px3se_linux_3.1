...
#define ADV7180_SENSOR_BUS_PARAM	(V4L2_MBUS_MASTER |     \
...
static int adv7180_vehicle_status(struct ad_dev *ad,
				  u32 *status,
				  v4l2_std_id *std)
{
	unsigned char status1 = 0;

	status1 = vehicle_generic_sensor_read(ad, ADV7180_STATUS1_REG);
	//DBG("ADV7180_STATUS1_REG: %x\n", status1);
	if (status1 < 0)
		return status1;
...
static void adv7180_reinit_parameter(struct ad_dev *ad, v4l2_std_id std)
{
	int i;

	ad->cfg.bus_param = ADV7180_SENSOR_BUS_PARAM;
	if (std == V4L2_STD_PAL) {
...
static void adv7180_reg_init(struct ad_dev *ad, unsigned char cvstd)
{
...
int adv7180_ad_get_cfg(struct vehicle_cfg **cfg)
{
	...
	adv7180_vehicle_status(g_addev, &status, NULL);
	if (status) /* No signal */
...
static int adv7180_check_std(struct ad_dev *ad, v4l2_std_id *std)
{
	u32 status;
	static bool is_first = true;

	adv7180_vehicle_status(ad, &status, std);
...
static void adv7180_check_state_work(struct work_struct *work)
{
	...
	adv7180_check_std(ad, &std);

	if (std != std_old) {
		std_old = std;
		adv7180_reinit_parameter(ad, std);
...
int adv7180_ad_init(struct ad_dev *ad)
{
	...
	if (0) {
		...
		ret = request_irq(ad->irq, NULL, IRQF_TRIGGER_FALLING,
				  "vehicle ad_adv7180", ad);
	}

	/*  5. create workqueue to detect signal change */
	INIT_DELAYED_WORK(&ad->state_check_work.work, adv7180_check_state_work);
	ad->state_check_work.state_check_wq =
		create_singlethread_workqueue("vehicle-ad-adv7180");
