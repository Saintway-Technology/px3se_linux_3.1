...
#define SENSOR_V4L2_IDENT V4L2_IDENT_ADV7180
...
#define ADV7180_INPUT_CONTROL		0x00
#define ADV7180_INPUT_DEFAULT		0x00
#define ADV7180_INPUT_CVBS_AIN2		0x00
#define ADV7180_INPUT_CVBS_AIN3		0x01
#define ADV7180_INPUT_CVBS_AIN5		0x02
#define ADV7180_INPUT_CVBS_AIN6		0x03
#define ADV7180_INPUT_CVBS_AIN8		0x04
#define ADV7180_INPUT_CVBS_AIN10	0x05
#define ADV7180_INPUT_CVBS_AIN1		0x0B
#define ADV7180_INPUT_CVBS_AIN4		0x0D
#define ADV7180_INPUT_CVBS_AIN7		0x0F
#define ADV7180_INPUT_YPRPB_AIN6_8_10	0x09
...
static int now_channel_value = ADV7180_INPUT_CVBS_AIN1;
...
static void adv7180_reinit_parameter(v4l2_std_id std,
				     struct generic_sensor *sensor);
static void adv7180_send_uevent(struct generic_sensor *sensor);
...
static inline int adv7180_channel_set(struct i2c_client *client,
				      ...
				      struct v4l2_ext_control *ext_ctrl)
{
	...
	if (sensor->channel_id != ext_ctrl->value) {
		...
		if (strstr(channel_infos.channel_info[id], "cvbs")) {
			...
			switch (channel_ain) {
			case 1:
				now_channel_value = ADV7180_INPUT_CVBS_AIN1;
				break;
			case 2:
				now_channel_value = ADV7180_INPUT_CVBS_AIN2;
				break;
			case 3:
				now_channel_value = ADV7180_INPUT_CVBS_AIN3;
				break;
			case 4:
				now_channel_value = ADV7180_INPUT_CVBS_AIN4;
				break;
			case 5:
				now_channel_value = ADV7180_INPUT_CVBS_AIN5;
				break;
			case 6:
				now_channel_value = ADV7180_INPUT_CVBS_AIN6;
				break;
			case 7:
				now_channel_value = ADV7180_INPUT_CVBS_AIN7;
				break;
			case 8:
				now_channel_value = ADV7180_INPUT_CVBS_AIN8;
				break;
			case 10:
				now_channel_value = ADV7180_INPUT_CVBS_AIN10;
				break;
			}
			...
			adv7180_reinit_parameter(V4L2_STD_NTSC, sensor);
		} else if (strstr(channel_infos.channel_info[id], "yprpb")) {
			now_channel_value = ADV7180_INPUT_DEFAULT;
			sensor_series[i].data = sensor_preview_data_yprpb_p;
			sensor->info_priv.dev_sig_cnf.type =
				RK_CAMERA_DEVICE_BT601_8;
			sensor->info_priv.dev_sig_cnf.code =
				V4L2_MBUS_FMT_YVYU8_2X8;
			adv7180_reinit_parameter(V4L2_STD_525_60, sensor);
		} else if (strstr(channel_infos.channel_info[id], "ycrpc")) {
			now_channel_value = ADV7180_INPUT_YPRPB_AIN6_8_10;
		...
		sensor_write(client, ADV7180_INPUT_CONTROL, now_channel_value);
					    ...
					    bool is_set)
{
	...
	{
		if (is_set) {
			ret = adv7180_channel_set(client, ctrl_info, ext_ctrl);
			if (ret)
				goto cb_end;
		} else {
...
static void adv7180_send_uevent(struct generic_sensor *sensor)
{
	...
	event_msg = kasprintf(GFP_KERNEL,
			      "CVBS_NAME=ADV7180 NOW_INPUT_MODE=%s RESOLUTION=%dx%d",
			  ...
			  v4l2_std_id *std)
{
	...
	ret = sensor_read(client, ADV7180_STATUS1_REG, &status1);

	if (IS_ERR_VALUE(ret)) {
		SENSOR_TR("%s(%d): register read failed: 0x%x\n",
			  __func__, __LINE__, ADV7180_STATUS1_REG);
		return status1;
	}
...
static ssize_t adv7180_debugfs_reg_write(struct file *file,
...
static ssize_t adv7180_debugfs_reg_read(struct file *file,
					char __user *buf,
					size_t count, loff_t *ppos)
{
...
static int adv7180_debugfs_reg_show(struct seq_file *s, void *v)
{
	int i, ret;
	unsigned char val;
...
static int adv7180_debugfs_open(struct inode *inode, struct file *file)
{
	struct specific_sensor *spsensor = inode->i_private;

	return single_open(file, adv7180_debugfs_reg_show, spsensor);
}

static const struct file_operations debugfs_register_fops = {
	.owner			= THIS_MODULE,
	.open			= adv7180_debugfs_open,
	.read			= seq_read,
	.write			= adv7180_debugfs_reg_write,
	.llseek			= seq_lseek,
...
static const struct file_operations debugfs_status_fops = {
	.owner			= THIS_MODULE,
	.open			= adv7180_debugfs_open,
	.read			= adv7180_debugfs_reg_read,
	.write			= adv7180_debugfs_reg_write,
	.llseek			= seq_lseek,
	.release		= single_release
};

static void adv7180_check_state_work(struct work_struct *work)
{
	...
	adv7180_status(client, &status, &std);
	if ((status != V4L2_IN_ST_NO_SIGNAL) && (std_old != std)) {
		adv7180_reinit_parameter(std, sensor);
		adv7180_send_uevent(sensor);
		std_old = std;
					...
					struct soc_camera_device *icd)
{
	...
	ret = request_irq(spsensor->common_sensor.irq, ADV7180_irq,
	...
	*/
	...
	/* init debugfs */
	debugfs_dir = debugfs_create_dir("adv7180", NULL);
