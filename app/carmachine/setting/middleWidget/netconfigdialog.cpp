...
netConfigDialog::netConfigDialog(QWidget *parent):QDialog(parent)
{
    ...
    connect(authSelect, SIGNAL(activated(int)),this,SLOT(authChanged(int)));
...
void netConfigDialog::paramsFromScanResults(netWork network)
{
    new_network = true;

    /* SSID BSSID frequency signal flags */
    setWindowTitle(network.SSID);
    ssidEdit->setText(network.SSID);

    QString flags = network.flags;
    int auth, encr = 0;
...
void netConfigDialog::eapChanged(int sel)
{
...
void netConfigDialog::wepEnabled(bool enabled)
{
...
void netConfigDialog::getEapCapa()
{
    char reply[256];
    size_t reply_len;

    reply_len = sizeof(reply) - 1;
    if (wpaManager::getInstance(this)->ctrlRequest("GET_CAPABILITY eap", reply, &reply_len) < 0)
        return;
    reply[reply_len] = '\0';
...
void netConfigDialog::addNetwork()
{
    ...
    setNetworkParam(id, "ssid", ssidEdit->text().toLocal8Bit().constData(),true);
...
void netConfigDialog::writeWepKey(int network_id, QLineEdit *edit, int id)
{
    char buf[10];
    bool hex;
    const char *txt, *pos;
...
void netConfigDialog::removeNetwork()
{
    char reply[10], cmd[256];
    size_t reply_len;
...
void netConfigDialog::paramsFromConfig(int network_id)
{
    ...
    reply_len = sizeof(reply) - 1;
    if (wpaManager::getInstance(this)->ctrlRequest(cmd, reply, &reply_len) >= 0 && reply_len >= 1)
    {
        reply[reply_len] = '\0';
        switch (atoi(reply)) {
        case 0:
